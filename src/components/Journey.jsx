...
export default function Journey() {
  ...
  return (
    <section id="journey" className="py-28 border-t border-zinc-900 bg-[#030303]">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        
        {/* Section Header */}
        <div className="mb-16 text-center">
          <p className="font-mono text-xs text-teal-400 uppercase tracking-widest mb-2">04. Milestones</p>
          <h2 className="text-3xl md:text-5xl font-black text-white tracking-tight">Building Journey</h2>
        ...
        {/* Timeline Path */}
        <div className="relative border-l-2 border-zinc-900 ml-4 md:ml-8 pl-8 space-y-12">
          {steps.map((step, idx) => (
            <div key={idx} className="relative group">
              {/* Glowing Connector Node */}
              <span className={`absolute -left-[43px] top-1.5 flex items-center justify-center w-8 h-8 rounded-full bg-zinc-950 border ${step.color} shadow-lg transition-transform group-hover:scale-110 duration-300`}>
                {step.icon}
              </span>
              ...
              <div className="p-6 rounded-lg border border-zinc-850 bg-zinc-900/10 hover:border-zinc-800 hover:bg-zinc-900/20 transition-all glow-card">
                <div className="flex flex-wrap items-center justify-between gap-2 mb-2">
                  <span className="px-2.5 py-0.5 rounded-full text-[10px] font-mono font-semibold bg-zinc-950 border border-zinc-850 text-zinc-400">
                    {step.year}
                  </span>
                ...
                <div className="flex items-center gap-1 text-sm font-mono text-teal-400 mb-4">
